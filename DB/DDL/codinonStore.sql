CREATE DATABASE codingon_store_db CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;
...
-- FULL OUTER JOIN union 이 중복을 제거한다.
SELECT c.customer_id, name, email, order_id, order_date, order_status
FROM customers c
LEFT JOIN orders o
ON c.customer_id = o.customer_id
...
FROM  customers c
	...
	ON c.customer_id = o.customer_id;
-- 4
SELECT c.name, p.name product_name
FROM  customers c
	CROSS JOIN  orders o
	ON c.customer_id = o.customer_id;
-- 5 여기서부터 모범답안
