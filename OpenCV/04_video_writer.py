...
# 2. VideoWriter 객체 생성
out = cv2.VideoWriter(
    f'{os.path.dirname(__file__)}/output/capture_video.mp4', fourcc, fps,(width,height))
...
while cap.isOpened():
    ...
    if cv2.waitKey(int(1000/fps)) == ord('q'):
        cv2.imwrite(f'{os.path.dirname(__file__)}/output/capture_video.jpg',frame) #누른 시점으로 사진 저장
        break
