...
- 미래에 실패/ 성공 에 대한 결과 값을 "약속" 한다는 의미
- 성공, 실패 분리해서 반환
    - 성공, 실패에 대한 결과는 then, catch 메서드로 이어 받아서 처리가능
-> 성공이든 실패이든 무언가의 "최종 결과"
*/
...
function promise1(flag){
    return new Promise(function(resolve, reject){
        if(flag){
            resolve(`현재 프로미스의 상태는 fulfilled(이행)! then 메서드를 연결~ 이 때 flag값은 ${flag}`)
        } else {
            reject(`현재 프로미스의 상태는 rejected(거절)! catch 메서드를 연결~ 이 때 flag값은 ${flag}`)
        }
    });
}
