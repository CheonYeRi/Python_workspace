#include <MFRC522.h>
#include <SPI.h> //단거리간 통신 방법 중 하나

#define RST_PIN 9
#define SS_PIN 10

int R_LED = 3;
int B_LED = 4;
int PIEZO = 7;

MFRC522 myMFRC(SS_PIN, RST_PIN); // MFRC522 라이브러리를 사용해서 나만의 myMFRC 객체 생성

void setup() {
  Serial.begin(9600);

  pinMode(3, OUTPUT);
  pinMode(4, OUTPUT);
  pinMode(7, OUTPUT);

  SPI.begin(); // SPI 통신 초기화(실행)
              // SPI 통신: 하나의 master(아두이노)와 다수의 slave(종속적인 역할, RFID리더) 간의 통신 방식
  myMFRC.PCD_Init(); //RFID 리더기 초기화(실행)

  myMFRC.PCD_DumpVersionToSerial(); // 연결 확인용 버전 정보 출력 코드
  Serial.println("RFID 통신 준비 완료");
}

void loop() {
  digitalWrite(R_LED, LOW);
  digitalWrite(B_LED, LOW);
  digitalWrite(PIEZO, LOW);

  // 1. 근처에 태그가 있는 지 확인
  if(!myMFRC.PICC_IsNewCardPresent()) 
    return;  //새로운 카드 없을 시 리턴
  // 2. 근처 태그의 UID 확인
  if(!myMFRC.PICC_ReadCardSerial())
    return;
  
  Serial.print("Card UID: ");

  
  //태그의 UID값을 받아와 출력
  // 아두이노는 한 번의 통신에 1바이트만 통신이 가능하기 때문에
  // 근처 태그의 UID 길이 만큼 반복문을 통해서 시리얼 모니터에 출력
  for (byte i = 0; i < myMFRC.uid.size; i++) {
    Serial.print(myMFRC.uid.uidByte[i]);
    Serial.print(" ");
  }

  Serial.println(""); // 하나의 태그 UID 출력 완료시 줄바꿈
  int isWhiteCard = (myMFRC.uid.uidByte[0] == 164 && myMFRC.uid.uidByte[1] == 156 
      && myMFRC.uid.uidByte[2] == 80 && myMFRC.uid.uidByte[3] == 5); 

  if (isWhiteCard == true) {
    digitalWrite(B_LED, HIGH);
    digitalWrite(PIEZO, HIGH);
    delay(500);
    digitalWrite(PIEZO, LOW);
    digitalWrite(B_LED, LOW);
    return;
  } else {
    digitalWrite(R_LED, HIGH);
    digitalWrite(PIEZO, HIGH);
    delay(500);
    digitalWrite(PIEZO, LOW);
    delay(500);
    digitalWrite(PIEZO, HIGH);
    delay(500);
    digitalWrite(PIEZO, LOW);
    digitalWrite(R_LED, LOW);

  }

  // 통신 종료 (통신 종료를 안 하면 다른 루프에서 근처의 태그 UID를 인식하지 못하는 경우 존재.)
  myMFRC.PICC_HaltA();
  myMFRC.PCD_StopCrypto1();

  delay (1000); //연속 인식 방지
}